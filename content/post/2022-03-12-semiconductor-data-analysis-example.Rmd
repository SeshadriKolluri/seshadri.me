...
df_baseline %>% group_by(Wafer) %>% summarise(Yield_Percentage = sum(Pass)/n()) %>% 
  ggplot(aes(x = Wafer, y = Yield_Percentage)) + 
  geom_bar(stat = "identity", fill = 'darkseagreen') + 
  scale_y_continuous(labels = scales::percent) + scale_x_continuous(breaks = (1:6)) +  
  geom_text(aes(label = scales::percent(Yield_Percentage, accuracy = 1)), nudge_y = 0.06 ) + 
